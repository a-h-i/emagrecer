...
export class CreateComputeRecipeTotalsFunc1756077439079 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
      ...
      begin
        ...
        select 
          coalesce(sum( (nutr_per_100g->>'kcal')::int * grams / 100), 0),
        ...
        update recipe set
          kcal_per_serving = round(kcal / greatest(servings, 1))::int,
          protein_g_per_serving = round(protein / greatest(servings, 1), 2),
          carbs_g_per_serving = round(carbs / greatest(servings, 1), 2),
          fat_g_per_serving = round(fat / greatest(servings, 1), 2)
        where id = recipe_id;
